...
const deleteUser = async (req, res, next) => {
  try {
    ...
    if (result.rowCount === 0) {
      return res.status(404).json({ message: 'Book no found--delete' });
    }
...
const updateUser = async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE users SET full_name=$1, cellphone=$2, address=$3, roles=$4, email=$5, password=$6 WHERE isbn=$7 RETURNING *;',
      [full_name, cellphone, address, role, email, password, id]
    );

    if (result.rows.length === 0) {
      return res.status(404).json({ message: 'book not found--update' });
    }
    return res.json(result.rows[0]);
  } catch {
    next(error);
